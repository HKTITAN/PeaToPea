...
- [ ] **1.1** Create root directory layout
  - [ ] 1.1.1 Add `pea-core/` directory (Rust crate: protocol reference implementation)
  - [ ] 1.1.2 Add `pea-windows/` directory (PeaPod protocol implementation for Windows)
  - [ ] 1.1.3 Add `pea-android/` directory (PeaPod protocol implementation for Android)
  - [ ] 1.1.4 Add `pea-linux/` directory (PeaPod protocol implementation for Linux)
  - [ ] 1.1.5 Add `pea-ios/` directory (PeaPod protocol implementation for iOS)
  - [ ] 1.1.6 Add `pea-macos/` directory (PeaPod protocol implementation for macOS)
  - [ ] 1.1.7 Add root `Cargo.toml` workspace if using Rust workspace
- [ ] **1.2** Root config files
  - [ ] 1.2.1 Add `.gitignore` (target/, build/, .idea/, *.iml, etc.)
  - [ ] 1.2.2 Add root `README.md` with project name (PeaPod: Project PeaToPea) and link to .tasks
  - [ ] 1.2.3 Add `.editorconfig` or format config if desired

## 2. Version control and branching

- [ ] **2.1** Initialize Git (if not already)
  - [ ] 2.1.1 `git init`
  - [ ] 2.1.2 Create initial commit with README and .tasks
- [ ] **2.2** Branching strategy
  - [ ] 2.2.1 Document main/develop (or trunk) in README or CONTRIBUTING
  - [ ] 2.2.2 Define feature branch naming (e.g. `feature/pea-core-identity`)

## 3. Rust toolchain (for pea-core and desktop clients)

- [ ] **3.1** Rust setup
  - [ ] 3.1.1 Ensure `rustup` and stable toolchain available
  - [ ] 3.1.2 Add `rust-toolchain.toml` or document required version in README
  - [ ] 3.1.3 Add targets if needed: `aarch64-apple-ios`, `x86_64-apple-ios`, `aarch64-linux-android`, etc.
- [ ] **3.2** Cargo workspace (optional)
  - [ ] 3.2.1 Create root `Cargo.toml` with `[workspace]` and members: `pea-core`, `pea-windows`, `pea-linux`
  - [ ] 3.2.2 Ensure pea-core can be built alone and as dependency
...
- [ ] **6.1** Core and Windows/Linux CI
  - [ ] 6.1.1 Add CI config (e.g. GitHub Actions) to build `pea-core` on push/PR
  - [ ] 6.1.2 Run `cargo test` for pea-core
...
- [ ] **6.4** Linting and format
  - [ ] 6.4.1 Run `cargo fmt -- --check` for Rust
  - [ ] 6.4.2 Run `cargo clippy` for pea-core (and other Rust crates)
